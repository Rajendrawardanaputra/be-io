...
class DetailTimelineListCreateAPIView(ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        for detail_timeline in detail_timeline_list:
            # Logging activity for each detail_timeline
            user_id = detail_timeline.id_user.id_user
            self.log_activity(user_id, 'created', 'DetailTimeline', detail_timeline)

        return Response(serializer.data, status=status.HTTP_201_CREATED)

    def log_activity(self, user_id, action, name_table, detail_timeline, new_photo=None, old_photo=None):
...
class DetailTimelineDetailAPIView(RetrieveUpdateDestroyAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        old_data = model_to_dict(instance)  # Ambil data lama sebelum perubahan
    ...
    def perform_update(self, serializer, old_data):
        ...
        serializer.save()
        self.log_activity(user_id, 'updated', 'DetailTimeline', old_data=old_data)

    def perform_destroy(self, instance):
        user_id = instance.id_user.pk
        old_data = model_to_dict(instance)  # Ambil data lama sebelum dihapus
        instance.delete()
        self.log_activity(user_id, 'deleted', 'DetailTimeline', old_data=old_data)
        return Response({"message": "DetailTimeline berhasil dihapus"}, status=status.HTTP_204_NO_CONTENT)

    def log_activity(self, user_id, action, name_table, old_data, new_data=None):
        user_instance = get_object_or_404(User, id_user=user_id)
